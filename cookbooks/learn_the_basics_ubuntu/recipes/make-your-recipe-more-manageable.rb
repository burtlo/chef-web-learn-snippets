...
with_snippet_options(cwd: '~/chef-repo', step: 'create-a-cookbook') do
  ...
  snippet_execute 'chef-generate-cookbook' do
    command 'chef generate cookbook cookbooks/learn_chef_apache2'
    # trim_stdout ({
    #   from: /\s+\* template\[\/root\/chef-repo\/cookbooks\/learn_chef_apache2\/metadata\.rb.*?restore selinux security context/m,
    #   to: /\* directory\[\/root\/chef-repo\/cookbooks\/learn_chef_apache2\/recipes\] action create.*?restore selinux security context/m
    #   })
  end
...
with_snippet_options(cwd: '~/chef-repo', step: 'create-a-template') do
  ...
  snippet_execute 'chef-generate-template' do
    command 'chef generate template cookbooks/learn_chef_apache2 index.html'
  end
  ...
  snippet_code_block 'index-1' do
    file_path '~/chef-repo/cookbooks/learn_chef_apache2/templates/index.html.erb'
...
with_snippet_options(cwd: '~/chef-repo', step: 'update-the-recipe-to-reference-the-html-template') do
  ...
  snippet_code_block 'default-1' do
    file_path '~/chef-repo/cookbooks/learn_chef_apache2/recipes/default.rb'
...
with_snippet_options(cwd: '~/chef-repo', step: 'run-the-cookbook') do
  ...
  snippet_execute 'cookbook-ccr-1' do
    command "sudo chef-client --local-mode --runlist 'recipe[learn_chef_apache2]'"
  end
